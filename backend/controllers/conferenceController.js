...
const getAllConferences = async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT c.*, u.name as creator_name FROM Conferences c 
...
const createConference = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO Conferences (title, description, topics, venue, submission_deadline, created_by) VALUES (?, ?, ?, ?, ?, ?)',
      [title, description, topics, venue, submission_deadline, req.user.id]
    );
...
const publishConference = async (req, res) => {
  try {
    const { id } = req.params;
    await db.query("UPDATE Conferences SET status = 'published' WHERE id = ?", [id]);
...
const updateConference = async (req, res) => {
  try {
    ...
    await db.query(
      'UPDATE Conferences SET title=?, description=?, topics=?, venue=?, submission_deadline=?, status=? WHERE id=?',
...
module.exports = { getAllConferences, getAllConferencesAdmin, getConferenceById, createConference, publishConference, updateConference };
